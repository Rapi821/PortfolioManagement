...
const sellStocks = asyncHandler(async (req, res) => {
  ...
  const c = (await persons.getStackCount(req.body, req.session.user.user_id))[0]
    .count;
  // console.log(c - req.body.count);
  if (c - req.body.count < 0) {
    res
      .status(500)
      .json("Du hast nicht so viele Aktien wie du verkaufen willst!");
    return;
  } else if (c - req.body.count > 0) {
    ...
    await persons.sellStocksCompletely(req.body, req.session.user.user_id);
  }
  res
    .status(200)
    .json(await persons.addToRecords(req.body, req.session.user.user_id));
...
const changeStatus = asyncHandler(async (req, res) => {
  res
    .status(200)
    .json(await persons.changeStatus(req.params.competition_id, req.body));
});
