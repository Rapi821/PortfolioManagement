...
const getCompetitionsByUser = async (user_id) => (await query("select cm.member_id, c.title, (select sum(buy_price) as cash from competition_member_depot_lines where member_id= 1 and isin != '0000') as portfolio_value, cmdl.buy_price, (select sum(buy_price) from competition_member_depot_lines where member_id= 1) as total, c.active from competition_members cm JOIN competitions c on c.competition_id= cm.competition_id JOIN competition_member_depot_lines cmdl on c.competition_id = cmdl.competition_id where cm.user_id=$1 and cmdl.isin= '0000';", [user_id])).rows;
